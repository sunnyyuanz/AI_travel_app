...
const Itinerary = () => {
  ...
  useEffect(() => {
    ...
    if(generatedItinerary && generatedTitle && generatedTotalCosts){
      ...
      const fetchItinerary = async () => {
        try {
          const response = await axios.post('http://13.59.28.106:5000/generate_itinerary', req);
  ...
  const updateItinerary = async () => {
    ...
    if(userSuggestion.length>0){
      setIsLoading(true)
      try {
        const response = await axios.post("http://13.59.28.106:5000/update_itinerary", {
  ...
  const onSaveClick = async () => {
    ...
    if(id && id.length>0){
      ...
      console.log(image)
      try {
        const response = await axios.post("http://3.22.130.213:4000/api/itinerary/save-itinerary", {
          itinerary,
          userId: id,
